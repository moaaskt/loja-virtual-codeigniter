...
<div class="card shadow-sm border-0 rounded-3 mb-4">
    <div class="card-body p-4">
        <form action="<?= site_url('admin/produtos/update/' . $produto['id']) ?>" method="post" enctype="multipart/form-data" id="form-produto">
            <?= csrf_field() ?>
        ...
        <div class="row g-4">
            ...
            <div class="col-md-8">
                <div class="form-check form-switch mt-4 pt-2">
                    <input class="form-check-input" type="checkbox" name="frete_gratis" id="frete_gratis" value="1" <?= old('frete_gratis', $produto['frete_gratis'] ?? 0) ? 'checked' : '' ?>>
                ...
                <div class="border rounded-3 p-4 bg-light shadow-sm">
                    ...
                    <div class="d-flex flex-wrap justify-content-between align-items-center mb-3 gap-2">
                        <div>
                            ...
                            <p class="text-muted small mb-0">Gerencie SKUs individuais, fotos específicas por cor e gere novas combinações em grade.</p>
                        </div>
                        <div class="d-flex gap-2">
                            ...
                            <button type="button" class="btn btn-sm btn-outline-secondary rounded-pill px-3" id="btn-add-atributo">
                                <i class="bi bi-plus-circle me-1"></i>Configurador de Grade
                            </button>
                        </div>
                    </div>
                    ...
                    <div id="container-atributos" class="p-3 bg-white border rounded-3 mb-3" style="display: none;">
                        ...
                        <div class="d-flex justify-content-between align-items-center pt-2 border-top">
                            ...
                            <button type="button" class="btn btn-primary btn-sm rounded-pill px-4 fw-semibold shadow-sm" id="btn-gerar-grade">
                                <i class="bi bi-magic me-1"></i>⚡ Gerar / Substituir Grade de SKUs
                            </button>
                        </div>
                    </div>

                    <!-- BARRA DE AÇÕES EM LOTE (BULK EDIT) -->
                    <div id="bulk-edit-bar" class="p-2 px-3 bg-white border rounded-3 mb-3 d-flex flex-wrap align-items-center justify-content-between gap-2" style="<?= empty($variacoes) ? 'display: none;' : 'display: flex;' ?>">
                    ...
                    <div class="table-responsive border rounded-3 bg-white shadow-sm">
                        <table class="table table-hover align-middle mb-0" id="tabela-variacoes" style="<?= empty($variacoes) ? 'display: none;' : '' ?>">
                            ...
                            <tbody id="tbody-variacoes">
                                <?php if (!empty($variacoes)): ?>
                                    <?php foreach ($variacoes as $index => $var): ?>
                                        <?php 
                                        $corValor    = $var['cor'] ?? '';
                                        $corHexDb    = $var['cor_hex'] ?? '';
                                        $nomeVar     = !empty($var['nome_variacao']) ? $var['nome_variacao'] : (!empty($var['tamanho']) ? $var['tamanho'] : ($corValor ?: 'Variação ' . ($index+1)));
                                        $sku         = $var['sku'] ?? '';
                                        $attrJson    = !empty($var['atributos_json']) ? $var['atributos_json'] : (!empty($var['atributos']) ? json_encode($var['atributos'], JSON_UNESCAPED_UNICODE) : '');
                                        $tamanho     = $var['tamanho'] ?? '';
                                        $imagemUrl   = $var['imagem_url'] ?? '';
                                        
                                        $pickerColor = '#000000';
                                        if (preg_match('/^#[0-9a-fA-F]{6}$/i', $corHexDb)) {
                                            $pickerColor = $corHexDb;
                                        } elseif (preg_match('/^#[0-9a-fA-F]{6}$/i', $corValor)) {
                                            $pickerColor = $corValor;
                                        }
                                        ?>
                                        <tr>
                                            <td>
                                                <input type="hidden" name="variacoes[<?= $index ?>][id]" value="<?= esc($var['id']) ?>">
                                                <input type="hidden" name="variacoes[<?= $index ?>][nome_variacao]" value="<?= esc($nomeVar) ?>">
                                                <input type="hidden" name="variacoes[<?= $index ?>][atributos_json]" value="<?= esc($attrJson) ?>">
                                                <input type="hidden" name="variacoes[<?= $index ?>][tamanho]" value="<?= esc($tamanho) ?>">
                                                ...
                                                <div class="input-group input-group-sm">
                                                    ...
                                                    <input type="text" name="variacoes[<?= $index ?>][sku]" class="form-control form-control-sm font-monospace" value="<?= esc($sku) ?>" placeholder="Ex: SKU-123">
                                                </div>
                                            </td>
                                            <td>
                                                <div class="input-group input-group-sm">
                                                    <input type="color" name="variacoes[<?= $index ?>][cor_hex]" class="form-control form-control-color p-1 color-picker-input" value="<?= esc($pickerColor) ?>" style="width: 36px; height: 31px; cursor: pointer;" title="Cor">
                                                    <input type="text" name="variacoes[<?= $index ?>][cor]" class="form-control form-control-sm color-text-input" value="<?= esc($corValor) ?>" placeholder="Ex: Azul, Preto">
                                                </div>
                                            </td>
                                            <td>
                                                <div class="input-group input-group-sm">
                                                    <span class="input-group-text">R$</span>
                                                    <input type="number" step="0.01" min="0" name="variacoes[<?= $index ?>][preco]" class="form-control form-control-sm var-preco-input" value="<?= !empty($var['preco']) ? esc($var['preco']) : '' ?>" placeholder="Base">
                                                </div>
                                            </td>
                                            <td>
                                                <input type="number" name="variacoes[<?= $index ?>][estoque]" class="form-control form-control-sm var-estoque-input" value="<?= esc($var['estoque']) ?>" placeholder="0" min="0" required>
                                            </td>
                                            <td>
                                                <div class="input-group input-group-sm">
                                                    <input type="url" name="variacoes[<?= $index ?>][imagem_url]" class="form-control form-control-sm var-img-input" value="<?= esc($imagemUrl) ?>" placeholder="https://... foto da cor">
                        ...
                        <div id="variacoes-empty" class="text-center p-5 text-muted" style="<?= !empty($variacoes) ? 'display: none;' : '' ?>">
                            ...
                            <p class="small text-muted mb-3">Utilize os <strong>Presets Rápidos</strong> acima ou clique em <strong>Configurador de Grade</strong> para gerar as combinações automaticamente.</p>
                        </div>
                    </div>
                ...
                <div class="row g-3 align-items-start">
                    <div class="col-md-6">
                        <div class="p-3 border rounded-3 bg-light h-100">
                            <label for="imagem" class="form-label fw-semibold">Alterar Imagem Principal (Capa)</label>
                            <input class="form-control" type="file" id="imagem" name="imagem" accept="image/*">
                            <small class="text-muted d-block mt-1">Ou informe uma nova URL abaixo:</small>
                            <input type="url" name="url_imagem" class="form-control mt-2" placeholder="https://exemplo.com/imagem.jpg" value="<?= old('url_imagem', strpos($produto['imagem'] ?? '', 'http') === 0 ? $produto['imagem'] : '') ?>">

                            <label for="imagens" class="form-label fw-semibold mt-3">Adicionar Fotos à Galeria</label>
                                ...
                                <?php if (!empty($imagens)): ?>
                                    <?php foreach ($imagens as $img): ?>
                                        <div class="position-relative">
                                            ...
                                            <img src="<?= $isUrl ? esc($img['caminho_imagem']) : base_url('uploads/produtos/' . esc($img['caminho_imagem'])) ?>" class="rounded border shadow-sm" style="width:100px; height:100px; object-fit:cover;">
        ...
        <div class="d-flex gap-2 mt-4 pt-3 border-top justify-content-end">
            ...
            <button type="submit" class="btn btn-primary px-5 rounded-pill fw-semibold shadow-sm" id="btn-atualizar-produto">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    btnAddAtributo.addEventListener('click', function() {
        if (containerAtributos.style.display === 'none') {
            containerAtributos.style.display = 'block';
            if (atributosDefinidos.length === 0) {
                addAtributo('Cor', ['Azul', 'Preto']);
                addAtributo('Armazenamento', ['128GB', '256GB']);
            }
        } else {
            addAtributo('Novo Atributo', []);
        }
    });
    ...
    btnGerarGrade.addEventListener('click', function() {
        ...
        if (tbodyVariacoes.children.length > 0 && !confirm('Gerar uma nova grade substituirá as variações existentes da tabela. Deseja continuar?')) {
            return;
        }
    ...
    document.getElementById('btn-limpar-todas-variacoes').addEventListener('click', function() {
        if (confirm('Deseja realmente remover todas as variações da tabela?')) {
            tbodyVariacoes.innerHTML = '';
            checkEmptyState();
        }
    });
    ...
    const itensParaExcluir = document.getElementById('itens-para-excluir');
    document.querySelectorAll('.btn-remover-imagem').forEach(function(btn) {
